...
def main(config):
    ...
    def get_batch(index, contexts, start_token):
        ...
        remove_target = [random.random() < (1 - config.remove_percent) for _ in
                         range(len(target_sentence))]  # determine removal of original sentence as mention in paper
        target_context = [sen[:i] + sen[i + remove:] for i, sen, remove in
                          zip(target_sentence, sentence, remove_target)]  # set sentences of target context
